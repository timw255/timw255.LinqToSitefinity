...
{
    public class DocumentLibrariesRequestProcessor<T> : IRequestProcessor<T>
    where T : class
    {
        public List<T> QuerySitefinity(SitefinityRequest queryParameters, SitefinityRestClient sf)
        {
            LibraryViewModel[] items = null;

            DocumentLibraryServiceWrapper service = new DocumentLibraryServiceWrapper(sf);
